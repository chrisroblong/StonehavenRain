...
parser.add_argument('--rolling', type=int, nargs='+',
                    help='Additional rolling mean to apply to  data. Specifying nothing means rolling will not be done. If specified each period will be done.')
...
parser.add_argument('--coarsen', type=int, help='Regrids data in both axes by number given')
...
if args.verbose:
    print("Processing")
dsmax = process(pr_coarsen).load()  # process and load  the raw data
...
if args.rolling is not None:
    ...
    for roll in args.rolling:
        ...
        pr = pr_coarsen.rolling(time=roll).mean()
