...
This is for short return period/low intensity events are in the bulk of the GEV distribution,
...
The ratio of the intensity ratio to the Clausius-Clapeyron scaling, given by the dotted lines,
    ...
    given in table~\ref{tab:CCtable}, multiplied by the CET difference as the return period tends to infinity.
...
This effect is likely to be mitigated by the fact that the Stonehaven event occurred over 4 hours,
    and so is more likely to be rarer and further into the tail of the distribution,
...
%  TODO assess risks specific to the railway
%  TODO mention different scaling of shape parameter
%  TODO mention fit to different times in the radar distribution
%  TODO process with rolling average
%  TODO process with different bins
%  TODO Pareto-Picklands model
%  TODO distribution of Stonehaven region instead of CET
%  TODO model event similarity

\subsection{Implications for event attribution}\label{subsec:disfield}

%  TODO mention Simon's paper may not be conservative for 2020 event w/ covariate shape

%  TODO mention scaling with Stonehaven region temp, not CET