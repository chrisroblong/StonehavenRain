...
3) Load radarMax data and find crash radar max for 2020-08 & 2021-08. Add to radar dataSet
...
def plot_ir(ax, all_ir, mc_all_ir, plot_styles, dist_now, qt=0.95, q=[0.05, 0.95],
    ...
    sec_ax_rain.set_xlabel(f'Radar Rx1h (mm/hr)', fontsize='small')
...
def plot_pr(ax_pr, all_pr, mc_all_pr, plot_styles, dist_now, qt=0.95, q=[0.05, 0.95], crit_value=None):
    ...
    for k, plot_Args in plot_styles.items():
        ...
        ax_pr.fill_between(mc_all_pr_range['Rx1h'],
                           mc_all_pr_range.isel(quantile=0),
                           mc_all_pr_range.isel(quantile=1),
                           color=col, alpha=0.5)
        if crit_value is not None:
            pr = float(all_pr[k].sel(time_quant=qt).interp(Rx1h=crit_value))
            prmin = float(mc_all_pr_range.isel(quantile=0).interp(Rx1h=crit_value))
            prmax = float(mc_all_pr_range.isel(quantile=1).interp(Rx1h=crit_value))
...
def plot_pr_sens(ax, all_pr, plot_styles, crit_values, qt=0.95):
    ...
    for k, plot_args in plot_styles.items():  # iterate over time periods.
        pr = all_pr[k].sel(Rx1h=crit_values.sel(time_quant=tq), method='nearest').sel(time_quant=tq)
        pr.plot(ax=ax, x='Rx1h', **plot_args, linewidth=2, marker='o', ms=6)
        prr = pr.sel(time_quant=qt)  # plot the specified regional extreme with a large hexagon rather than a dot
        ax.plot(float(prr.Rx1h), float(prr), color=plot_args['color'], marker='h', ms=12)
...
def plot_result(all_ir, all_pr, mc_all_ir, mc_all_pr, temperatures, cc_scale, cc_rng, mn_ratio_roll, dist_now, radar,
                plot_styles, num=None, qt=0.95):
    """
    ...
    cv = float(radar.critical2020.sel(time_quant=qt))
    ...
    plot_pr_sens(ax_sens[1], all_pr, plot_styles,
                 radar.critical2020.sel(time_quant=slice(0.05, 0.95)))  # plot sens case for probability ratios
    ...
    for rolling, linestyle in zip(mn_ratio_roll['rolling'].values, linestyles):
        all_ir_roll, all_pr_roll, dist_now_roll, scales_roll = comp_intense_prob_ratios(radar_fit.Parameters,
                                                                                        mn_ratio_roll.sel(
                                                                                            rolling=rolling),
                                                                                        rain, rtn_period,
                                                                                        temperatures,
    ...
    rp_to_rain, rain_to_rp = gen_inv_fns(
        dist_now)  # need the inverse functions to convert from ranfall to return period
    ...
    for v, ls in zip([radar.crash2020], ['solid']):
    ...
    for a, title, xtitle, ytitle in zip(axis, ['Intensity Ratio Increase (%) ', 'Probability Ratio'] * 2,
                                        ['Return Period (Years)', f' {radar_time} (mm/hr)'] * 2,
                                        ['% increase', 'PR'] * 2):
        a.set_title(title)
        a.set_xlabel(xtitle)
        a.set_ylabel(ytitle)
...
crash2021 = float(ts.sel(time='2021-08'))
crash2020 = float(ts.sel(time='2020-08'))
radar['crash2021'] = crash2021
radar['crash2020'] = crash2020
rgn = 'CET'  # want CET covariate.
## get in the data -- only want the rolling =1 cases = no rolling!

dirend = coarsening + '_gev_fits_roll'
outdir_gev = stonehavenRainLib.dataDir / dirend
...
t_ref = float(
    obs_cet.sel(time=(obs_cet.time.dt.month == 7)).sel(time=slice('2005', '2020')).mean())  # mn temp for radar data
...
temperatures = {'PI': t_pi, '2012-2021': t_today, '2005-2020': t_ref, '1961-1990': t_1961_1990, '1980s': t_1980s,
                'PI+2K': temp_p2k.mean()}
npt = 200
rain = np.linspace(5, 200, npt)
rtn_period = np.geomspace(5, 200, npt)
all_ir, all_pr, dist_now, scales = comp_intense_prob_ratios(radar_fit.Parameters, mn_ratio_roll.sel(rolling=1), rain,
                                                            rtn_period,
                                                            temperatures, scale_ref='2005-2020')
dist_now = xarray.Dataset(dict(Parameters=dist_now))
all_ir_shape, all_pr_shape, dist_now_shape, scales_shape = comp_intense_prob_ratios(radar_fit.Parameters,
                                                                                    mn_ratio_shape_roll.sel(rolling=1),
                                                                                    rain, rtn_period,
                                                                                    temperatures, scale_ref='2005-2020')
...
mc_all_ir, mc_all_pr = pr_ir_monte_carlo(mc_radar_fit, bootstrap_ratio.sel(rolling=1), temperatures, temp_p2k, rain,
                                         rtn_period,
                                         file_pattern=str(outdir_gev) + "/", refresh=refresh)
mc_shape_all_ir, mc_shape_all_pr = pr_ir_monte_carlo(mc_radar_fit, bootstrap_ratio_shape.sel(rolling=1), temperatures,
                                                     temp_p2k, rain, rtn_period,
...
figShape = plot_result(all_ir_shape, all_pr_shape, mc_shape_all_ir, mc_shape_all_pr, temperatures, cc_scale, cc_rng,
                       mn_ratio_shape_roll,
